...
export const RichOutputWithSidebar: React.FC<RichOutputWithSidebarProps> = React.memo(({
  ...
  return (
    <div className="flex h-full relative">
      ...
      {/* Toggle Button - positioned lower to avoid settings overlap */}
      <button
        onClick={() => setIsCollapsed(!isCollapsed)}
        className={cn(
          'absolute top-14 z-10 bg-surface-secondary hover:bg-surface-hover',
          'border border-border-primary rounded-l-lg p-1.5',
      ...
      >
        {isCollapsed ? (
          <>
            <History className="w-3.5 h-3.5 text-text-tertiary group-hover:text-text-secondary" />
            <ChevronLeft className="w-3.5 h-3.5 text-text-tertiary group-hover:text-text-secondary" />
          </>
        ) : (
          <ChevronRight className="w-3.5 h-3.5 text-text-tertiary group-hover:text-text-secondary" />
        )}
      </button>
