...
import { Star, Archive, Crosshair } from 'lucide-react';
...
export const SessionListItem = memo(function SessionListItem({ session, isNested = false }: SessionListItemProps) {
  ...
  const isSpotlighted = useSessionStore(state => state.isSpotlightActive(session.id));
  ...
  const handleToggleSpotlight = useCallback(async (e: React.MouseEvent) => {
    e.stopPropagation();
    try {
      if (isSpotlighted) {
        const result = await window.electronAPI.spotlight.disable(session.id);
        if (!result.success) {
          alert(result.error || 'Failed to disable spotlight');
        }
      } else {
        const result = await window.electronAPI.spotlight.enable(session.id);
        if (!result.success) {
          alert(result.error || 'Failed to enable spotlight');
        }
      }
    } catch (error) {
    ...
    <>
      <div
        className={cn(
          ...
          isSpotlighted && 'ring-1 ring-amber-400/50'
        )}
      ...
      >
        ...
        <div className="flex items-center space-x-1">
          {!isEditing && (
            <>
              {!session.archived && !session.isMainRepo && (
                <IconButton
                  ...
                  aria-label={isSpotlighted ? 'Disable Spotlight' : 'Enable Spotlight'}
                  icon={
                    <Crosshair
                      className="w-4 h-4"
                      strokeWidth={isSpotlighted ? 2.5 : 2}
                    />
                  }
                />
              )}
